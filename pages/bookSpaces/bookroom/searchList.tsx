...
const BookRoom = () => {
  ...
  const [spacedetailsLength, setSpacedetailsLength] = useState<number>(0);
  ...
  const processSearchRequest = async () => {
    const spaceDetails: IAvailableSpaceDetail[] =   await getSpaceBySearchCriteria(searchBookingState);
    ...
    const uniqueSpaceIds = new Set(spaceDetails.map(space => space.spaceId));
    const uniqueSpaceIdCount = uniqueSpaceIds.size;
    setSpacedetailsLength(uniqueSpaceIdCount);
  ...
  const checkAndAddSpaceToFloor = (
    ...
    // Check if spaceType is '1' before processing
    if (spaceDetail.spaceType === '1') {
      if (
          ...
          <div className="flex items-center sm:w-full">
            <FormControl className="min-w-[8em]">
              <FormControlLabel
                control={
                <Checkbox
                    checked={searchBookingState.allDays}
                    onChange={() =>
                      setSearchBookingState({
                        ...searchBookingState,
                        allDays: !searchBookingState.allDays,
                      })
                    }
                  />}
                label="All Day"
            ...
            <FormControl className="sm:w-full">
              ...
              <Select 
                 labelId="reminderLabel" 
                  label="Reminders" className="min-w-[12rem] sm:w-[full]"
                value={
                  roomData.reminders.find(
                    (remainder) => remainder.id === searchBookingState.reminder
                  )?.name
                }
                onChange={(e) => {
                  setSearchBookingState({
              ...
              <FormControl>
                <DateTimePicker
                 className="text-sm sm:w-full"
                  ...
                  disabled={searchBookingState.allDays}
                  onChange={(newValue) => {
                    setSearchBookingState({
                      ...searchBookingState,
                      startDate: newValue,
              ...
              <div className="flex flex-wrap gap-3">
                  {searchResult &&
                    ...
                    Object.keys(
                      ...
                      if (
                        searchResult[selectedFloorId].availableSpaceDetails[x].isAvailable &&
                        searchResult[selectedFloorId].availableSpaceDetails[x].spaceResources?.some(
                          (resource: ISpaceResource) =>
                            resource.facilityName?.toLowerCase() ===
                              String(spaceResourcesDictionary.Chair).toLowerCase() &&
                            resource.resourceCount >= searchBookingState.attendies
                        ) || searchResult[selectedFloorId].availableSpaceDetails[x].isAvailable)
                     {
                        return (
                          <CardComponent
